// used the javascript file provided by Professor Jeffery Jones in Week7
  ...
  */
  ...
  var offline = function() {
    if (!$('#status').hasClass('offline')) {
      $('#status')
        .attr('class', 'offline')
        .text('No Internet Connection! Please connect your device with internet to purchase the book.');
    }
  }
  ...
  // Loop the test every ten seconds for "test.html"
  
  function start() {
    rand = Math.floor(Math.random()*90000) + 20000;
    testConnection('  http://nwbuc.thewiredserf.com/test.html?fresh=' + rand);
